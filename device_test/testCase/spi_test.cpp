...
//		ts = td;
		...
		if(spi_read_len > 0)
		{
//			printf("queue_buff.size = %d  write spi_read_len=%d\n",queue_buff.size(),spi_read_len);
			queue_buff.push_back_block(buff, spi_read_len);
		}
...
//
////	auto  logger_ = spd::rotating_logger_mt("logger_",filename, 1048576 * 5, 3);
