...
SpiDevice::SpiDevice(const char * p_spidev){
  m_spidev = NULL;
  if (p_spidev != NULL ){
...
SpiDevice::SpiDevice(const char * p_spidev, spi_config_t *p_spi_config){
  m_spidev = NULL;
  if (p_spidev != NULL ){
      m_spidev = (char *)malloc(strlen(p_spidev)+1);
      if (m_spidev != NULL) 
